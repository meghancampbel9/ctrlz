...
if PRIVATE_KEY_PATH:
    try:
        ...
        print(f"[CRITICAL ERROR] Private key file not found at path: {PRIVATE_KEY_PATH}. Ensure the PRIVATE_KEY env var is set to a valid file path.")
    except Exception as e:
        print(f"[CRITICAL ERROR] Error reading private key file: {e}")

if not APP_ID or not WEBHOOK_SECRET or not PRIVATE_KEY:
    print("[CRITICAL ERROR] APP_ID, WEBHOOK_SECRET, or PRIVATE_KEY is not configured. Application may not function correctly.")
...
def parse_codefixer_output_to_files(llm_output: str) -> list[dict[str, str]]:
    ...
    for item in all_matches:
        ...
        if match_type == 'delete':
            ...
            if file_path_begin_str and file_path_begin_str == file_path_end_str:
                ...
                print(f"[CodeFixer Output Parser] Parsed UPSERT for: {file_path_begin_str} (Content length: {len(content_str)})")
            else:
                print(f"[CodeFixer Output Parser Warning] Mismatched or empty file paths in BEGIN/END markers: BEGIN='{file_path_begin_str}', END='{file_path_end_str}'")
    ...
    if not operations and llm_output.strip() not in ["NO_CODE_FIX_POSSIBLE", ""] and not llm_output.strip().startswith("# LLM_RESPONSE_UNEXPECTED_FORMAT"):
        ...
        print(f"[CodeFixer Output Parser Warning] No operations parsed from LLM output, and it's not a known special string. Output (first 500 chars): {llm_output[:500]}")
...
async def github_webhook(
    ...
    if not all([owner, repo_name, installation_id]):
        print(f"Error: Missing owner, repo_name, or installation_id in payload. Event: {x_github_event}, Action: {action}")
    ...
    if x_github_event == "repository" and action == "created":
        ...
        if status == "completed" and conclusion in negative_conclusions:
            print(f"Workflow run {run_id} in {owner}/{repo_name} ended with negative status '{conclusion}'. Initiating CodeFixer.")
            ...
            try:
                ...
                if owner and repo_name and workflow_path and installation_id: # current_head_sha already checked
                    print(f"[INFO] Attempting to fetch workflow YAML '{workflow_path}' from '{owner}/{repo_name}' at SHA '{current_head_sha}'")
                    ...
                    if workflow_file_details and workflow_file_details.get("content") and workflow_file_details.get("encoding") == "base64":
                        ...
                        print(f"[INFO] Fetched workflow YAML '{workflow_path}'. Length: {len(workflow_yaml_content)} chars.")
                    else:
                        print(f"[WARN] Failed to fetch or decode workflow YAML content for {workflow_path} at {current_head_sha}. Proceeding without workflow content for CodeFixer.")
                        workflow_yaml_content = "" # Ensure it's an empty string if fetch fails
                else:
                    print(f"[WARN] Could not attempt to fetch workflow YAML. Missing components (owner, repo, path, or install_id).")
                ...
                if current_head_branch:
                    ...
                    print(f"[WARN] 'head_branch' missing in workflow_run payload for run {run_id}. Using default branch '{target_branch_for_fix}' as target for fix.")
            ...
            if not (os.getenv("GOOGLE_API_KEY") and embeddings_model and supabase):
                print("[ERROR] Core services (Google API Key, Embeddings, Supabase) not initialized. Skipping CodeFixer.")
            ...
            try:
                ...
                if codefixer_prompt_input.startswith("Error:"):
                    print(f"LogAnalyzer returned an error for run {run_id}, skipping RAG and CodeFixer. Error: {codefixer_prompt_input}")
                    return {"ok": True, "message": "LogAnalyzer error, CodeFixer skipped."} # Acknowledge, but don't proceed to fix

                current_repo_id_for_search = f"{owner}/{repo_name}"
                search_query_text = ""
                try:
                    problem_statement_start = codefixer_prompt_input.find("## Problem Statement")
                    key_snippets_start = codefixer_prompt_input.find("## Key Log Snippets") # Used as end marker for problem statement
                    if problem_statement_start != -1:
                        problem_statement_text_block = codefixer_prompt_input[problem_statement_start + len("## Problem Statement"):]
                        if key_snippets_start > problem_statement_start:
                            problem_statement_text_block = problem_statement_text_block[:key_snippets_start - (problem_statement_start + len("## Problem Statement"))]
                    ...
                    if not search_query_text: # Fallback if parsing above fails
                        ...
                        if temp_query_lines and temp_query_lines[0].strip() == "## Problem Statement":
                            start_line_index = 1
                        temp_query = "\n".join(temp_query_lines[start_line_index : start_line_index + 5]) # Take a few lines
                        search_query_text = temp_query[:500].strip() # Limit length
                        print(f"Warning: Could not precisely parse 'Problem Statement' for RAG query. Using heuristic (first few lines): '{search_query_text[:100]}...'")
                except Exception as e_parse:
                    print(f"Error parsing LogAnalyzer output for RAG query: {e_parse}. Using full output (truncated) as fallback.")
                    search_query_text = codefixer_prompt_input[:1000].strip() # Limit length

                full_file_code_context_for_fixer = []
                if search_query_text:
                    print(f"[DEBUG] RAG Step 1: Searching for relevant code CHUNKS. Query: '{search_query_text[:200]}...'")
                    ...
                    if relevant_chunks_from_rag:
                        ...
                        for chunk_info in relevant_chunks_from_rag:
                            ...
                            if file_path_from_chunk and (file_path_from_chunk, commit_sha_for_file_content) not in processed_files_for_full_content:
                                print(f"[DEBUG] RAG Step 2: Attempting to fetch full content for '{file_path_from_chunk}' at commit '{commit_sha_for_file_content}'")
                                ...
                                if full_content is not None: # Explicitly check for None, as empty string is valid content
                                    ...
                                    print(f"[DEBUG] RAG Step 2: Successfully fetched full content for {file_path_from_chunk} (length {len(full_content)}).")
                                    processed_files_for_full_content.add((file_path_from_chunk, commit_sha_for_file_content))
                                else:
                                    print(f"[WARN] RAG Step 2: Could not fetch full content for {file_path_from_chunk} at commit {commit_sha_for_file_content}. It will not be passed to CodeFixer.")
                ...
                if not full_file_code_context_for_fixer:
                    print("[INFO] RAG: No full file content could be retrieved for CodeFixer context (either no chunks found initially or full content fetch failed for all). CodeFixer will proceed without RAG context from codebase files.")
                ...
                if is_actionable_codefixer_output:
                    if owner and repo_name and installation_id and run_id and target_branch_for_fix and current_head_sha:
                        ...
                        if not file_operations:
                            print(f"[WARN] CodeFixer provided output, but parsing yielded no file operations. Output was: {proposed_fix_or_analysis[:500]}... Skipping PR creation.")
                        else:
                            try:
                                print(f"[GITHUB_ACTION] Attempting to delete branch {new_branch_name} if it exists (for idempotency).")
                                await delete_github_branch(owner, repo_name, new_branch_name, installation_id)

                                print(f"[GITHUB_ACTION] Creating new branch for fix: {new_branch_name} from SHA {current_head_sha}")
                                ...
                                for op in file_operations:
                                    ...
                                    print(f"[DEBUG] Fetching original file details for '{file_path_op}' at commit SHA '{current_head_sha}' (branch '{target_branch_for_fix}') for '{commit_action}' operation.")
                                    ...
                                    if original_file_details and "sha" in original_file_details:
                                        ...
                                        if commit_action == 'delete':
                                            print(f"[ERROR] Cannot delete '{file_path_op}': original file SHA not found (or file does not exist on branch '{target_branch_for_fix}' at commit '{current_head_sha}'). Skipping delete operation for this file.")
                                            continue # Skip this delete operation
                                        else: # Upsert for a new file if SHA not found
                                            print(f"[INFO] No SHA found for '{file_path_op}' during '{commit_action}'. Assuming new file creation.")
                                            original_file_sha_for_op = None # Explicitly set to None for new file

                                    if commit_action == "upsert":
                                        file_content = op["content"]
                                        print(f"[DEBUG] Committing ('{commit_action}') file '{file_path_op}' (SHA: {'NEW' if not original_file_sha_for_op else original_file_sha_for_op}) to branch '{new_branch_name}'")
                                        ...
                                        if original_file_sha_for_op: # SHA is mandatory for delete
                                            print(f"[DEBUG] Deleting file '{file_path_op}' (SHA: {original_file_sha_for_op}) from branch '{new_branch_name}'")
                                ...
                                if not committed_file_paths and not deleted_file_paths:
                                    print("[INFO] No files were successfully committed or deleted by CodeFixer based on its output. Skipping PR creation.")
                                else:
                                    # These joins correctly use single \n and will produce strings with actual newlines
                                    pr_body_files_list_committed = "\n".join([f"- Modified/Created: `{f}`" for f in committed_file_paths])
                                    pr_body_files_list_deleted = "\n".join([f"- Deleted: `{f}`" for f in deleted_file_paths])
                                    ...
                                    if "## Problem Statement" in codefixer_prompt_input:
                                        try:
                                            ...
                                            if problem_end_idx == -1: problem_end_idx = len(codefixer_prompt_input) # if no key snippets
                                            parsed_problem = codefixer_prompt_input[problem_start_idx:problem_end_idx].strip()
                                            if parsed_problem: problem_statement_for_pr = parsed_problem[:800] + ("..." if len(parsed_problem) > 800 else "") # Limit length
                                    ...
                                    print(f"[DEBUG] Creating PR: head='{new_branch_name}', base='{base_branch_for_pr}', title='{pr_title}'")
                                ...
                                print(f"[ERROR] Failed to apply changes and create GitHub pull request for run {run_id}: {e_gh_pr}")
                                import traceback
                                traceback.print_exc()
                    else: # Missing critical info for GitHub ops
                        print(f"[INFO] Skipping PR creation for run {run_id} due to missing GitHub operational information (owner, repo, installation_id, current_head_sha, etc.).")
                ...
                elif proposed_fix_or_analysis and proposed_fix_or_analysis.startswith("# LLM_RESPONSE_UNEXPECTED_FORMAT"):
                    print(f"[WARN] CodeFixer output for run {run_id} was in an unexpected format. No PR will be created. Details logged previously.")

                else: # Error from CodeFixer or empty/unhandled response
                    print(f"[INFO] No actionable output from CodeFixer for run {run_id}. Skipping PR creation. Response: '{proposed_fix_or_analysis}'")

            except Exception as e_agent_pipeline: # Catch errors from agent pipeline
                print(f"[CRITICAL] Unhandled error during LogAnalyzer, RAG, or CodeFixer execution for run {run_id}: {e_agent_pipeline}")
                import traceback
                traceback.print_exc()

        else: # Workflow run not completed or did not fail negatively
            print(f"Workflow run {run_id} in {owner}/{repo_name} action '{action}', status '{status}', conclusion '{conclusion}'. No action taken by CodeFixer.")
