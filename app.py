...
async def get_github_file_details(owner: str, repo: str, path: str, ref: str, installation_id: str) -> dict | None:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(f"[GitHub API Error] Failed to get file details for {path} at ref {ref} in {owner}/{repo}. Status: {e.response.status_code}, Response: {e.response.text}")
            return None
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code == 404 or resp.status_code == 422: # 404 or 422 (if ref doesn't exist for deletion)
                print(f"[GitHub API] Branch {branch_name} did not exist or couldn't be deleted (Status: {resp.status_code}). Safe to proceed with creation.")
            else:
            ...
            if e.response.status_code == 404 or e.response.status_code == 422:
                print(f"[GitHub API] Branch {branch_name} not found during delete attempt (Status: {e.response.status_code}). Safe to create.")
            else:
...
async def commit_file_to_github(owner: str, repo: str, branch_name: str, file_path: str, file_content: str, commit_message: str, installation_id: str, original_file_sha: str):
    token = await get_installation_access_token(installation_id)
                ...
                if hunk_line.startswith('+'):
                    ...
                    if original_line_idx < len(original_lines):
                        ...
                        if actual_original_line == expected_context_from_diff:
                            patched_lines.append(original_lines[original_line_idx]) # Append the true original line with its original spacing
                            original_line_idx += 1
                            diff_line_idx += 1
                        else:
                        ...
                        print(f"[ERROR] Diff context line implies original content beyond its actual length. Original lines: {len(original_lines)}, current index: {original_line_idx}. Diff line: '{hunk_line}'")
            ...
            try:
                ...
                if owner and repo_name and workflow_path and current_head_sha and installation_id:
                    ...
                    if workflow_file_details and workflow_file_details.get("content") and workflow_file_details.get("encoding") == "base64":
                        ...
                        print(f"[WARN] Fetched workflow YAML details, but content was not base64 encoded or was missing: {workflow_file_details}")
                    ...
                    if not current_head_sha: print(f"[WARN] Missing: current_head_sha (from payload.workflow_run.head_sha)")
                ...
                if current_head_branch:
                    ...
                    print("[WARN] head_branch is missing from payload.workflow_run.head_branch. Target branch will remain '{target_branch_for_fix}'.")
                    ...
                    if not codefixer_prompt_input.startswith("Error:"):
                        ...
                        try:
                            ...
                            if problem_statement_start != -1:
                                problem_statement_end = key_snippets_start if key_snippets_start > problem_statement_start else len(codefixer_prompt_input)
                                search_query_text = codefixer_prompt_input[problem_statement_start + len("## Problem Statement"):problem_statement_end].strip()
                            
                            if not search_query_text: 
                                temp_query = codefixer_prompt_input.split('\\n')[1] if len(codefixer_prompt_input.split('\\n')) > 1 else codefixer_prompt_input
                                search_query_text = temp_query[:500] 
                                print(f"Warning: Could not precisely parse Problem Statement for RAG query, using heuristic: '{search_query_text[:100]}...'")
                        
                        except Exception as e_parse:
                            print(f"Error parsing LogAnalyzer output for RAG query: {e_parse}. Using full output as fallback.")
                            search_query_text = codefixer_prompt_input
                        
                        print(f"[DEBUG] Parsed search_query_text (first 50 chars): '{search_query_text[:50]}' ") # DIAGNOSTIC 2b
                        
                        if search_query_text:
                            print(f"[DEBUG] About to call search_relevant_code_chunks. Query: '{search_query_text[:200]}...'") # DIAGNOSTIC 3
                            ...
                            if relevant_code_context:
                                ...
                                for i, context_chunk in enumerate(relevant_code_context):
                                    print(f"--- Snippet {i+1} (Similarity: {context_chunk.get('similarity'):.4f}) ---")
                                ...
                                print(f"[DEBUG] CodeFixer instantiated. About to call propose_fix. workflow_yaml defined: {workflow_yaml_content is not None}, target_branch defined: {target_branch_for_fix is not None}") # DIAGNOSTIC 6
                                ...
                                if proposed_fix_or_analysis and "NO_CODE_FIX_POSSIBLE" not in proposed_fix_or_analysis and "```diff" in proposed_fix_or_analysis:
                                    if owner and repo_name and installation_id and run_id and target_branch_for_fix and target_branch_for_fix != "unknown" and current_head_sha:
                                        print(f"[INFO] Valid fix proposed. Attempting to apply changes and create a PR for run {run_id} in {owner}/{repo_name}.")
                                        ...
                                        cleaned_diff = proposed_fix_or_analysis.replace("```diff", "").replace("```", "").strip()
                                        individual_file_diffs = parse_diff_to_files(cleaned_diff)

                                        if not individual_file_diffs:
                                            print("[WARN] Could not parse any file diffs from CodeFixer output. Skipping PR creation.")
                                        else:
                                            try:
                                                print(f"[GITHUB_ACTION] Attempting to delete branch {new_branch_name} if it exists.")
                                                ...
                                                print(f"[GITHUB_ACTION] Creating branch: {new_branch_name} from SHA {current_head_sha}")
                                                ...
                                                for file_diff_info in individual_file_diffs:
                                                    ...
                                                    print(f"[DEBUG] Fetching original file details for {file_path_to_patch} at SHA {current_head_sha}")
                                                    original_file_details = await get_github_file_details(
                                                        owner, repo_name, file_path_to_patch, current_head_sha, installation_id
                                                    )
                                                    if not original_file_details or "content" not in original_file_details or "sha" not in original_file_details:
                                                        print(f"[ERROR] Could not fetch original file details (content/SHA) for {file_path_to_patch} from ref {current_head_sha}. Skipping this file.")
                                                        continue
                                                    ...
                                                    if original_file_details.get("encoding") != "base64":
                                                        print(f"[ERROR] Original file {file_path_to_patch} encoding is not base64. Skipping this file.")
                                                        continue
                                                    try:
                                                        original_content_str = base64.b64decode(original_content_b64).decode('utf-8')
                                                    except Exception as e_decode:
                                                        print(f"[ERROR] Failed to decode base64 content for {file_path_to_patch}: {e_decode}. Skipping this file.")
                                                        continue

                                                    # 2. Apply the patch
                                                    print(f"[DEBUG] Applying patch to {file_path_to_patch}")
                                                    patched_content_str = apply_patch_to_content(original_content_str, specific_diff_content)
                                                    
                                                    if patched_content_str == "PATCH_APPLICATION_FAILED_CONTEXT_MISMATCH":
                                                        print(f"[ERROR] Failed to apply patch for {file_path_to_patch} due to context mismatch. Skipping commit for this file.")
                                                        # Optionally, remove from modified_file_paths if we don't want to list it in PR
                                                        if file_path_to_patch in modified_file_paths:
                                                            modified_file_paths.remove(file_path_to_patch) # So it won't be listed in PR body
                                                    ...
                                                    print(f"[DEBUG] Committing patched file {file_path_to_patch} to branch {new_branch_name}")
                                                    await commit_file_to_github(
                                                        ...
                                                        installation_id=installation_id,
                                                        original_file_sha=original_file_sha # Pass the original_file_sha
                                                    )
                                                    print(f"[GITHUB_ACTION] Successfully committed patched {file_path_to_patch} to {new_branch_name}.")

                                                if not modified_file_paths:
                                                    print("[INFO] No files were successfully patched and committed. Skipping PR creation.")
                                                else:
                                                    pr_body_files_list = "\\n".join([f"- `{f}`" for f in modified_file_paths])
                                                    pr_body = (f"This PR applies changes proposed by the CodeFixer agent for the workflow failure on branch `{target_branch_for_fix}` (triggered by commit `{current_head_sha[:7]}`).\\n\\n"
                                                               f"**Files Modified:**\\n{pr_body_files_list}\\n\\n"
                                                               f"Please review the applied changes.")

                                                    print(f"[DEBUG] Creating PR: head={new_branch_name}, base={base_branch_for_pr}")
                                        ...
                                        print(f"[INFO] Skipping PR creation for run {run_id} due to missing information (owner, repo, installation_id, run_id, target_branch_for_fix, or current_head_sha).")
                                else:
                                    print(f"[INFO] No valid fix proposed by CodeFixer or fix was 'NO_CODE_FIX_POSSIBLE' for run {run_id}. Skipping PR creation.")
                            else:
                                print(f"LogAnalyzer returned an error, skipping RAG search and CodeFixer: {codefixer_prompt_input}")
